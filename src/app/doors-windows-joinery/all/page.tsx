...
export default async function AllDoorsWindowsJoineryPage({
  searchParams,
}: {
  searchParams: Record<string, string | string[] | undefined>;
}) {
  const { category, products: allProducts } =
    await getCategoryAllProductsData("doors-windows-joinery");
  ...
  return (
    <div className="bg-cream font-sans text-ink antialiased">
      ...
      <ListingView
        breadcrumb={[
          { label: "Home", href: "/" },
          { label: title, href: "/doors-windows-joinery" },
          { label: "All" },
        ]}
        title={`All ${title}`}
        description="Every published piece in Doors, Windows & Joinery, in one place."
        products={products}
        emptyMessage={listingEmptyMessage(
          allProducts.length,
          "No Doors, Windows & Joinery pieces published yet — check back soon.",
        )}
