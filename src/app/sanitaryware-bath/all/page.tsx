...
export default async function AllSanitarywareBathPage({
  searchParams,
}: {
  searchParams: Record<string, string | string[] | undefined>;
}) {
  const { category, products: allProducts } =
    await getCategoryAllProductsData("sanitaryware-bath");
  ...
  return (
    <div className="bg-cream font-sans text-ink antialiased">
      ...
      <ListingView
        breadcrumb={[
          { label: "Home", href: "/" },
          { label: title, href: "/sanitaryware-bath" },
          { label: "All" },
        ]}
        title={`All ${title}`}
        description="Every published piece across every Sanitarywares subcategory, in one place."
        products={products}
        emptyMessage={listingEmptyMessage(
          allProducts.length,
          "No Sanitarywares pieces published yet — check back soon.",
        )}
