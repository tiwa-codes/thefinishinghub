...
export default async function AllTilesWallFinishesPage({
  searchParams,
}: {
  searchParams: Record<string, string | string[] | undefined>;
}) {
  const { category, products: allProducts } =
    await getCategoryAllProductsData("tiles-wall-finishes");
  ...
  return (
    <div className="bg-cream font-sans text-ink antialiased">
      ...
      <ListingView
        breadcrumb={[
          { label: "Home", href: "/" },
          { label: title, href: "/tiles-wall-finishes" },
          { label: "All" },
        ]}
        title={`All ${title}`}
        description="Every published piece in Tiles & Wall Finishes, in one place."
        products={products}
        emptyMessage={listingEmptyMessage(
          allProducts.length,
          "No Tiles & Wall Finishes pieces published yet — check back soon.",
        )}
