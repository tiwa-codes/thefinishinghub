...
export default async function OfficeFurniturePage({
  ...
  const { category, products: allProducts } =
    await getSubcategoryListingData("furniture-office");
    ...
    <div className="bg-cream font-sans text-ink antialiased">
      ...
      <ListingView
        breadcrumb={[
          ...
          { label: category?.name ?? "Office" },
        ]}
        title="Office Furniture"
        description="Office furniture and storage for professional spaces — in stock at the Abuja showroom or made to order."
        products={products}
        emptyMessage={listingEmptyMessage(
          allProducts.length,
          "No Office pieces published yet — check back soon.",
        )}
