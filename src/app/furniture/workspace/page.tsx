...
export default async function WorkspaceFurniturePage({
  searchParams,
}: {
  searchParams: Record<string, string | string[] | undefined>;
}) {
  const { category, products: allProducts } =
    await getSubcategoryListingData("furniture-workspace");
    ...
    <div className="bg-cream font-sans text-ink antialiased">
      ...
      <ListingView
        breadcrumb={[
          ...
          { label: category?.name ?? "Workspace" },
        ]}
        title="Workspace Furniture"
        description="Desks, home-office seating and workspace storage — in stock at the Abuja showroom or made to order."
        products={products}
        emptyMessage={listingEmptyMessage(
          allProducts.length,
          "No Workspace pieces published yet — check back soon.",
        )}
