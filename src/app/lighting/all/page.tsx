...
export default async function AllLightingPage({
  searchParams,
}: {
  searchParams: Record<string, string | string[] | undefined>;
}) {
  const { category, products: allProducts } = await getCategoryAllProductsData("lighting");
  ...
  return (
    <div className="bg-cream font-sans text-ink antialiased">
      ...
      <ListingView
        breadcrumb={[
          { label: "Home", href: "/" },
          { label: title, href: "/lighting" },
          { label: "All" },
        ]}
        title={`All ${title}`}
        description="Every published piece in Lighting & Automation, in one place."
        products={products}
        emptyMessage={listingEmptyMessage(
          allProducts.length,
          "No Lighting pieces published yet — check back soon.",
        )}
